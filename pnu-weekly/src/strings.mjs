...
const ko = {
  ...
  legMarker: '크기 = 주목도(주간 언급량) · 색 = 위험신호 비율',
  ...
  // 범례 소제목 — 전에는 일곱 가지가 한 문단에 이어져 필요한 항목을 찾을 수 없었다.
  legIssueT: '이번 주 쟁점', legMarkerT: '마커 읽는 법', legLinesT: '선 읽는 법',
  legFactorT: '교란·완충 요인', legUseT: '다루는 법',
  legDisturb: '교란 요인', legBuffer: '완충',

  secNet: '주간 키워드 네트워크',
  netSub: (n, e, basis) => `국내 기사 ${basis}건의 제목에서 추출 · 키워드 ${n}개 · 연결 ${e}개`,
  // 항목별로 끊어 둔다. 줄글로 이으면 필요한 한 가지를 찾을 수 없다(지도 범례와 같은 이유).
  netPlanesT: '판 읽는 법',
  netPlanes: '<b>교육과 산업, 두 판</b>을 비스듬히 놓고 봅니다. 노드는 자기 판 위에 앉고, 점선 기둥이 어느 판인지 알려 줍니다. ' +
    '<b>판을 가로지르는 선이 곧 교육–산업 연계</b>입니다 — 그림에서 가장 눈에 띄는 것이 가장 중요한 정보가 되도록 판을 둘로 나눴습니다.',
  netDotsT: '점 읽는 법',
  netDots: '점은 자리를 잡고, <b>옅은 후광의 크기가 언급 기사 수</b>입니다. 후광 색 = 분야, 점 색 = 위험신호. ' +
    '색은 소분류를 따릅니다 — 교육은 차가운 색(정책/철학 · 융합연구 · 증강인재교육), 산업은 따뜻한 색(적응형행정 · AX 기술 동향).',
  netPickT: '고르는 기준',
  netPick: (minEdge, minCount) =>
    `${minCount}회 이상 등장한 말만 남기고, 같은 기사 제목에 <b>${minEdge}회 이상 함께 나온 쌍</b>만 잇습니다. ` +
    '산업은 기사 수가 적어 자리를 몇 개 비워 둡니다 — 없으면 억지로 채우지 않습니다.',
  netUseT: '다루는 법',
  netUse: '점에 마우스를 올리면 그 분야 판과 이웃만 남고, 점을 누르면 아래 참조 기사 목록에서 그 키워드가 든 제목을 짚어 줍니다. ' +
    '<b>끌면 360° 돌아갑니다.</b> 한 번 누른 뒤 휠을 굴리면 커서 자리를 기준으로 확대·축소되고(Ctrl+휠은 누르지 않아도 됩니다), ' +
    '휴대전화에서는 두 손가락으로 벌리고 오므리면 됩니다. 두 번 누르면 처음 각도·크기로 돌아갑니다.',
  netNoteT: '만드는 방법',
  netNote: '기사 제목만 읽고 본문은 저장하지 않습니다(저작권). 좌표는 빌드 때 한 번 계산해 고정하므로 화면과 PDF 가 같은 그림입니다.',
  ...
  secArticlesDaily: '기사',
  // 분야별 상위 3건만 싣는다. '외 N건'은 펼칠 수 없으므로(나머지는 페이지에 없다) 미리 알린다.
  articlesCap: '날짜를 펼치면 그날 수집한 기사를 분야별로 나눠 보여 줍니다. 분야마다 상위 3건만 싣고 나머지는 건수로만 적습니다. 본문에 인용된 기사는 맨 아래 «참조 기사 원문»에 모두 있습니다.',
  foldOpen: '이번 주차 리포트', foldClosed: '리포트 펼치기',
...
// 추세는 데이터에 영문 코드로 들어온다. 화면에 그대로 내보내면 거기만 영어가 된다.
ko.trendWord = { rising: '상승', falling: '하강', stable: '보합', baseline: '기준 주차' };
