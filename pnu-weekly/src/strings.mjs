...
const ko = {
  ...
  netLegend: (minEdge, minCount) =>
    `<b>교육과 산업, 두 판</b>을 두고 비스듬히 봅니다. 노드는 자기 판 위에 놓이고, 점선 기둥이 어느 판인지 알려 줍니다. ` +
    `<b>판을 가로지르는 선이 곧 교육-산업 연계</b>입니다 — 이 그림에서 가장 눈에 띄는 것이 가장 중요한 정보가 되도록 판을 둘로 나눴습니다. ` +
    `색은 소분류입니다. 교육은 차가운 색(정책/철학 · 융합연구 · 증강인재교육), 산업은 따뜻한 색(적응형행정 · AX 기술 동향). ` +
    `점은 자리를, <b>옅은 후광 크기 = 언급 기사 수</b>를 나타냅니다 · 후광 색 = 분야 · 점 색 = 위험신호 · ` +
    `<b>판을 가로지르는 선만 진하게</b> 그렸습니다 — 분야 간 얽힘이 이 그림의 핵심입니다(같은 기사 제목에 ${minEdge}회 이상 함께 등장한 쌍만 잇습니다). ` +
    `산업은 기사 수가 적어 자리를 몇 개 남겨 둡니다 — 없으면 억지로 채우지 않습니다. ` +
    `${minCount}회 이상 등장한 말만 남깁니다. 점에 마우스를 올리면 그 분야 판과 이웃만 남고, 점을 누르면 그 주 참조 기사 목록에서 해당 키워드가 든 제목을 표시합니다. ` +
    `<b>끌면 360° 돌아갑니다.</b> 그림을 한 번 누른 뒤 휠을 굴리면 커서 자리를 기준으로 확대·축소되고(Ctrl+휠은 누르지 않아도 동작합니다), 휴대전화에서는 두 손가락으로 벌리고 오므리면 됩니다. 두 번 누르면 각도와 크기가 처음으로 돌아갑니다. 제목만 읽고 본문은 저장하지 않습니다(저작권). 좌표는 빌드 때 한 번 계산해 고정하므로 화면과 PDF 가 같은 그림입니다.`,
  netRisk: '위험신호 비율',
