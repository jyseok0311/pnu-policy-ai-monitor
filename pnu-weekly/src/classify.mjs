...
// 숫자를 건너뛰는 이유: '관리 부담 30% 줄였다' 처럼 수치가 끼면 뒤를 못 읽었다.
const NEG = /^[\s'"‘’“”]*(?:이|가|을|를|은|는|도|만|의|에|과|와)?[\s'"‘’“”]*(?:\d[\d.,%]*\s*)?(없이|없는|없다|없어|없음|없었|없도록|없애|없앤|아닌|아니라|해소|불식|종식|막았|막은|막는|막기|방지|피했|피하|씻|잠재웠|덜었|덜어|줄였|제로|경감|완화|낮췄|낮춰|낮추|대응|극복|타개|돌파|활용|보다|부지)/;

// 각 등급의 위험어. 문자열이면 그대로 찾고, 정규식이면 그 꼴일 때만 센다.
export const RISK = {
  crisis: [
    // 폐교 기사는 문 닫는 사건보다 '닫은 뒤 그 자리를 어떻게 쓸까'가 더 많다(8건 중 5건).
    { w: '폐교', unless: /부지|활용|거점|유치|매입|재탄생|리모델링|탈바꿈/ },
    // '합의' 를 NEG 에 두면 '통합 잠정 중단 합의서' 까지 꺼 버린다(중단은 실제로 일어난 일이다).
    // 합의로 끝나는 것이 좋은 소식인 쪽은 소송뿐이라 여기에만 예외를 둔다.
    { w: '소송', unless: /합의|취하|각하|승소/ },
    // 수시·정시 경쟁률 표 기사는 학과별 '미달상태'를 나열할 뿐 사건이 아니다.
    // '10.38대1' 같은 배수 표기가 이 형식의 표식이다.
    { w: '미달', unless: /경쟁률\]|수시경쟁률|정시경쟁률|\d+\.?\d*대\s?1/ },
    '폐과', '학과 폐지', '통폐합', '무산', '삭감', '파행', '퇴출',
    ...
    // 낱말 하나로는 안 보이고 붙어야 보이는 것들. '취소'·'기각' 을 단독으로 두면
    // 행사 취소·소송 기각까지 끌어오므로 앞말을 붙여 좁힌다.
    /(?:지정|선정|승인|인가)\s?취소/,
    /(?:이의신청|신청|요구)\s?기각|기각(?:돼|됐)/,
    /경영위기대학|재정지원제한대학/,
    /반발(?!력)/,                       // 반발력(물리)은 제외
    'closure', 'shut down', 'crisis', 'collapse', 'lawsuit', 'scrapped', 'axed', 'slashed'
  ],
  warning: [
    '감축', '우려', '갈등', '축소', '논란', '경고', '압박', '차질',
    { w: '하락', unless: /경쟁률\]|수시경쟁률|정시경쟁률|\d+\.?\d*대\s?1/ },
    '반대', '제외', '패싱', '중단', '사퇴', '규탄', '항의', '난항', '표류', '적발', '부실', '반납', '철회', '징계', '고발',
    '후폭풍', '파문', '한계대학',
    /D등급|최하위|미흡\s?등급/,        // 평가 결과. A·S등급은 반대 신호라 건드리지 않는다
...
function riskHit(text, pat) {
  // { w, unless } 꼴이면 제목 전체가 unless 에 걸릴 때 아예 세지 않는다.
  // 낱말 바로 뒤만 봐서는 못 거르는 것이 있다 — '폐교를 지역 활력 거점으로…폐교 활용 지원사업'
  // 은 첫 '폐교' 뒤가 '를' 이라 뒤만 보는 방식으로는 걸러지지 않는다.
  if (pat && pat.w) {
    if (pat.unless.test(text)) return null;
    return riskHit(text, pat.w);
  }
